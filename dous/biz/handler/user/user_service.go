...
func UserIfmtResponse(ctx context.Context, c *app.RequestContext) {
	...
	//返回用户相关信息
	workC, _ := v.Where(v.AuthorID.Eq(req.UserId)).Count()
	print("workC:")
	println(workC)
	faC, _ := l.Where(l.UserID.Eq(req.UserId), l.Cancel.Eq(0)).Count()
	var user_info = user.User{
		...
		Avatar:          "111",          //用户头像
		BackgroundImage: "222",          //用户个人页顶部大图
		Signature:       "我是个人简介", //个人简介
		TotalFavorited:  0,              //获赞数量
		WorkCount:       workC,          //作品数量
		FavoriteCount:   faC,            //点赞数量
		//false:未关注
	}
...
func UserRegisterResponse(ctx context.Context, c *app.RequestContext) {
	...
	u, m := query.User, query.User.WithContext(ctx)
	m = m.Where(u.Username.Eq(req.Username))
	var total int64
	total, err = m.Count()
	if err != nil || total != 0 {
		resp.StatusCode = 1
